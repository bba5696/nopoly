...
real set comes with, and off by default. The host sets both: houses from four to
thirty-two, hotels from none to eight. Thirty-two rather than twenty because the
big board is a different game — eight countries at four houses each is where
every set on Classic is full, and a table that wants the rule present but gentle
needs the room; hotels stay at eight, since there are only so many sets to crown.
Both are lobby-only like every setting, which is what makes them safe: a supply
can never shrink under buildings already standing. Zero hotels is a real variant
rather than an accident, and four houses becomes the top of every property. What it buys is not scarcity for its
